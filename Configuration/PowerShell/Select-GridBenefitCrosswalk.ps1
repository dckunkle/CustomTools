<#
    ...
    .SYNOPSIS
        ...
        This script imports benefit crosswalk data from the Benefit Grid into the Configuration database
...
begin
{
    $worksheet_name = "Benefit Class Crosswalk"
...
process
{
    ...
    {   
        ...
        $headings = @()
        $headings = @('Benefit_ID','Bright Benefit Name','Benefit Class Type','Bright Cost Share Category','BRIF_BC')
    ...
    #**************************************************************************************************
    # Ouptut the benefit crosswalk details
    #**************************************************************************************************
    try
    {
        if ($status -eq "")
        {
            ...
            while ($benefit_id -ne "")
            {
                # Make sure the Client Benefit ID is defauted so that it can be bound
                $ClientBenefitID = $worksheet.Cells.Item($row,5).Text
                if ($ClientBenefitID -eq "") { $ClientBenefitID = -1}
                
                # Make sure the Benefit Class Type is defaulted so that it can be bound
                $BenefitClassType = $worksheet.Cells.Item($row,3).Text
                if ($BenefitClassType -eq "") { $BenefitClassType = "0 free visit(s)"}
                ...
                [PSCustomObject]@{
                    ...
                    BenefitName = $worksheet.Cells.Item($row,2).Text
                    BenefitClassType = $worksheet.Cells.Item($row,3).Text
                    CostShareCategory = $worksheet.Cells.Item($row,4).Text
                    ClientBenefitID = $ClientBenefitID
                }

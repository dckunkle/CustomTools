<# ***************************************************************************************************
Purpose:    Used by Jenkins to invoke the Data Creator with the parameters provided
Parameters: 
...
#**************************************************************************************************
# Configure the connection and run the Data Creator
#**************************************************************************************************
try
{
    ...
    $SQLCommand.CommandText = "EXEC spDCAuto_CreateData '" + $test_case + "', '" + $email_address + "','" + $build_number + "','" + $job_name + "'"
...
catch
{
    #Let the user know something went wrong and fail the script
    Write-Host ""
    Write-Host "       There was an error while trying to run the Data Creator: " $Error[0]
    Exit 1
}

#**************************************************************************************************
# Determine the log id that was created and output the results of the Data Creator run
#**************************************************************************************************
try
{
    $SQLConnection = New-Object System.Data.SqlClient.SqlConnection
    ...
    $SQLCommand = New-Object System.Data.SqlClient.SqlCommand
    $SQLCommand.Connection = $SQLConnection
    $SQLCommand.CommandText = "SELECT log_id FROM DCLog WITH (NOLOCK)WHERE build_id=" + $build_number + " AND job_name ='" + $job_name + "'"
    $SQLReader = $SQLCommand.ExecuteReader()
...
#**************************************************************************************************
.\jenkins\PowerShell\Output-DataCreatorLog.ps1 -server_name $server_name -log_id $log_id

<# ***************************************************************************************************
Purpose:    Used by Jenkins to invoke the Data Deleter (UserID) with the parameters provided
Parameters: 
...
Write-Host ""
Write-Host "------------------------------------------------------------------------------------------------------------------------------------------------"
Write-Host " DATA DELETER - USER ID"
Write-Host "------------------------------------------------------------------------------------------------------------------------------------------------"
Write-Host ""
Write-Host "       -Log ID---Date Time-------------Server Name----------User ID----------------------------------Email Address------------------------------"
...
try
{
    ...
    $SQLCommand.CommandText = "EXEC spDDAuto_DeleteTestData '" + $user_id + "', '" + $email_address + "','" + $build_number + "','" + $job_name + "'"
...
$user_id         = $user_id.PadRight(40)
$email_address   = $email_address.PadRight(40)


Write-Host "       " $str_log_id $date $server_name $user_id $email_address
Write-Host ""

#**************************************************************************************************
# Output the details of the Data Creator run to the Jenkins Console
#**************************************************************************************************
.\jenkins\powershell\Output-DataDeleterUserLog.ps1 -server_name $server_name -log_id $log_id

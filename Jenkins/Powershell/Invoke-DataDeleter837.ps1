<# ***************************************************************************************************
Purpose:    Used by Jenkins to invoke the Data Deleter (837) with the parameters provided
Parameters: 
...
#**************************************************************************************************
# If the user selected EOC Delete Data check to make sure it is OK to proceed with the delete
#**************************************************************************************************
if ($action -eq 'EOC Delete Data') 
{
    try
    {
        ...
        $EOCCommand.Connection = $EOCConnection
        $EOCCommand.CommandText = "EXEC spDDAuto_CheckEOCDelete '" + $filename + "'"
        $EOCReader = $EOCCommand.ExecuteReader()

        while ($EOCReader.Read())
        {

            $eoc_records = $EOCReader.Item("EOC_Records")
        }

        #Cleanup
        $EOCReader.Close()
        $EOCCommand.Dispose()
        $EOCConnection.Dispose()
    }
    catch
    {
        #Let the user know something went wrong and fail the script
...
#**************************************************************************************************
if (($action -ne "List Files") -and ($eoc_records -eq "No"))
{
    try
    {
        ...
        $SQLCommand.CommandText = "EXEC spDDAuto_DeleteFileData837Wrapper  '" + $filename + "', '" + $test_case + "','" + $action + "','" + $email_address + "','" + $build_number + "','" + $job_name + "'"
...
if ($action -eq "List Files")
{
    ...
    try
    {
        ...
        $FilesCommand.CommandText = "SELECT DISTINCT TOP 50 file_name, CONVERT(VARCHAR(10), MAX(date_process_start), 101) AS str_file_date, MAX(date_process_start) AS file_date FROM File_Receive_Log WITH (NOLOCK) WHERE file_type = '837' GROUP BY file_name ORDER BY MAX(date_process_start) DESC"
    ...
    {
        Write-Host "          No 837 files have been loaded previously."
        Write-Host ""
    }
...
if (($action -ne "List Files") -and ($eoc_records -eq "No"))
{
    .\jenkins\powershell\Output-DataDeleter837Log.ps1 -server_name $server_name -log_id $log_id
}

if ($eoc_records -eq "Yes")
{

    Write-Host ""
    Write-Host "       There are EOC records associated with this file and the EOC Delete Data option was selected."
    Write-Host "       Aborting delete..."
    Write-Host ""
}
Write-Host "------------------------------------------------------------------------------------------------------------------------------------------------"
Write-Host " "

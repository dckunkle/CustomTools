/**************************************************************************************************
Name:       spPortal_MaintainInvoiceData
...
EXEC spPortal_MaintainInvoiceData 1287
***************************************************************************************************/
CREATE OR ALTER PROCEDURE dbo.spPortal_MaintainInvoiceData
      (@days		INT	= 30
	  ,@log_id		INT	= 999)
AS
BEGIN
...
DECLARE @category			VARCHAR(20)			= 'Invoice'
       ,@sql				VARCHAR(8000)
...
--*************************************************************************************************
IF OBJECT_ID('tempdb.dbo.#invoice_ids') IS NOT NULL
	BEGIN DROP TABLE #invoice_ids END

CREATE TABLE #invoice_ids
      (INVOICE_ID				NUMERIC(18,0))
...
--*************************************************************************************************
INSERT INTO #table_deletes (table_name, check_ids) VALUES ('INVOICE', 0)
INSERT INTO #table_deletes (table_name, check_ids) VALUES ('INVOICE_ADJUSTMENT', 1)
INSERT INTO #table_deletes (table_name, check_ids) VALUES ('INVOICE_PAYMENT', 1)
INSERT INTO #table_deletes (table_name, check_ids) VALUES ('INVOICE_PAYMENT', 1)
INSERT INTO #table_deletes (table_name, check_ids) VALUES ('INVOICE_REQUEST', 1)
INSERT INTO #table_deletes (table_name, check_ids) VALUES ('ACCOUNT_SUMMARY', 1)
			...
			IF @check_ids <> ''
				BEGIN
					...
					SELECT @sql = 'DELETE TN '
					            + '  FROM #invoice_ids X '
							    + '  JOIN ' + @table_name + ' TN '
								+ '    ON X.INVOICE_ID = TN.INVOICE_ID'
					BEGIN TRY
						--PRINT @sql
						EXEC (@sql)
						SELECT @records = @@ROWCOUNT
						IF @records <> 0 BEGIN EXEC spPortal_DeleteRecordLogDetail @log_id, @category, 'INVOICE_ID Match', @table_name, @records END
					...
					BEGIN CATCH
						...
						SELECT @err_msg = ERROR_MESSAGE()
						      ,@err_num = 100
						EXEC spPortal_DeleteRecordLogDetail @log_id, @category, 'INVOICE_ID Match', @table_name, @records, @err_num, @err_msg

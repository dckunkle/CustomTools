/**************************************************************************************************
Name:       spPortal_MaintainServicePaymentData
...
EXEC spPortal_MaintainServicePaymentData 601
***************************************************************************************************/
CREATE OR ALTER PROCEDURE dbo.spPortal_MaintainServicePaymentData
      (@days		INT	= 30
	  ,@log_id		INT	= 999)
AS
BEGIN
...
DECLARE @category			VARCHAR(20)			= 'ServicePayment'
       ,@sql				VARCHAR(8000)
...
--*************************************************************************************************
IF OBJECT_ID('tempdb.dbo.#service_payment_ids') IS NOT NULL
	BEGIN DROP TABLE #service_payment_ids END

CREATE TABLE #service_payment_ids
      (SERVICE_PAYMENT_ID					NUMERIC(18,0))
...
INSERT INTO #table_deletes (table_name, check_ids) VALUES ('SERVICE_PMT_SUPPL_AMOUNT',1)
INSERT INTO #table_deletes (table_name, check_ids) VALUES ('HEALTH_CARE_REMARK_CODES',1)
INSERT INTO #table_deletes (table_name, check_ids) VALUES ('SERVICE_PAYMENT_ADJ',0)

--*************************************************************************************************
-- Populate the mail IDs to specifically look for and delete
--*************************************************************************************************
INSERT INTO #service_payment_ids
      (SERVICE_PAYMENT_ID)
SELECT M.SERVICE_PAYMENT_ID
  FROM SERVICE_PAYMENT			M
 WHERE M.DELETED				= 'Y'
			...
			IF @check_ids <> ''
				BEGIN
					...
					SELECT @sql = 'DELETE TN '
					            + '  FROM #service_payment_ids M '
							    + '  JOIN ' + @table_name + ' TN '
								+ '    ON M.SERVICE_PAYMENT_ID = TN.SERVICE_PAYMENT_ID'
					BEGIN TRY

						EXEC (@sql)
						SELECT @records = @@ROWCOUNT
						IF @records <> 0 BEGIN EXEC spPortal_DeleteRecordLogDetail @log_id, @category, 'SERVICE_PAYMENT_ID Match', @table_name, @records END
					...
					BEGIN CATCH
						...
						SELECT @err_msg = ERROR_MESSAGE()
						      ,@err_num = 100
						EXEC spPortal_DeleteRecordLogDetail @log_id, @category, 'SERVICE_PAYMENT_ID Match', @table_name, 0, @err_num, @err_msg

/**************************************************************************************************
Name:       spPortal_MaintainEmployeeData
...
EXEC spPortal_MaintainEmployeeData 1293
***************************************************************************************************/
CREATE OR ALTER PROCEDURE dbo.spPortal_MaintainEmployeeData
      (@days		INT	= 30
	  ,@log_id		INT	= 999)
AS
BEGIN
...
DECLARE @category			VARCHAR(20)			= 'Employee'
       ,@sql				VARCHAR(8000)
...
--*************************************************************************************************
IF OBJECT_ID('tempdb.dbo.#employee_ids') IS NOT NULL
	BEGIN DROP TABLE #employee_ids END
...
--*************************************************************************************************
INSERT INTO #table_deletes (table_name, check_ids) VALUES ('EMPLOYEE',0)
INSERT INTO #table_deletes (table_name, check_ids) VALUES ('EMPLOYEE_REQUEST',1)
INSERT INTO #table_deletes (table_name, check_ids) VALUES ('REQUEST',1)
			...
			IF @check_ids <> ''
				BEGIN
					...
					SELECT @sql = 'DELETE TN '
					            + '  FROM #employee_ids X '
							    + '  JOIN ' + @table_name + ' TN '
								+ '    ON X.EMPLOYEE_ID = TN.EMPLOYEE_ID'
					BEGIN TRY
						--PRINT @sql
						EXEC (@sql)
						SELECT @records = @@ROWCOUNT
						IF @records <> 0 BEGIN EXEC spPortal_DeleteRecordLogDetail @log_id, @category, 'EMPLOYEE_ID Match', @table_name, @records END
					...
					BEGIN CATCH
						...
						SELECT @err_msg = ERROR_MESSAGE()
						      ,@err_num = 100
						EXEC spPortal_DeleteRecordLogDetail @log_id, @category, 'EMPLOYEE_ID Match', @table_name, @records, @err_num, @err_msg

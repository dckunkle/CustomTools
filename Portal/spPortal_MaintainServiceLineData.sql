/**************************************************************************************************
Name:       spPortal_MaintainServiceLineData
...
EXEC spPortal_MaintainServiceLineData 1287
***************************************************************************************************/
CREATE OR ALTER PROCEDURE dbo.spPortal_MaintainServiceLineData
      (@days		INT	= 30
	  ,@log_id		INT	= 999)
AS
BEGIN
...
DECLARE @category			VARCHAR(20)			= 'ServiceLine'
       ,@sql				VARCHAR(8000)
...
--*************************************************************************************************
IF OBJECT_ID('tempdb.dbo.#service_line_ids') IS NOT NULL
	BEGIN DROP TABLE #service_line_ids END

CREATE TABLE #service_line_ids
      (SERVICE_LINE_ID				NUMERIC(18,0))
...
--*************************************************************************************************
INSERT INTO #table_deletes (table_name, check_ids) VALUES ('SERVICE_LINE', 0)
INSERT INTO #table_deletes (table_name, check_ids) VALUES ('NDC_DETAIL', 1)
...
--*************************************************************************************************
INSERT INTO #service_line_ids
      (SERVICE_LINE_ID)
SELECT X.SERVICE_LINE_ID
  FROM dbo.SERVICE_LINE			X
 WHERE X.DELETED				= 'Y'
			...
			IF @check_ids <> ''
				BEGIN
					...
					SELECT @sql = 'DELETE TN '
					            + '  FROM #service_line_ids X '
							    + '  JOIN ' + @table_name + ' TN '
								+ '    ON X.SERVICE_LINE_ID = TN.SERVICE_LINE_ID'
					BEGIN TRY
						--PRINT @sql
						EXEC (@sql)
						SELECT @records = @@ROWCOUNT
						IF @records <> 0 BEGIN EXEC spPortal_DeleteRecordLogDetail @log_id, @category, 'SERVICE_LINE_ID Match', @table_name, @records END
					...
					BEGIN CATCH
						...
						SELECT @err_msg = ERROR_MESSAGE()
						      ,@err_num = 100
						EXEC spPortal_DeleteRecordLogDetail @log_id, @category, 'SERVICE_LINE_ID Match', @table_name, @records, @err_num, @err_msg

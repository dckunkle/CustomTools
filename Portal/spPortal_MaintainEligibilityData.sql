/**************************************************************************************************
Name:       spPortal_MaintainEligibilityData
...
EXEC spPortal_MaintainEligibilityData 601
***************************************************************************************************/
CREATE OR ALTER PROCEDURE dbo.spPortal_MaintainEligibilityData
      (@days		INT	= 30
	  ,@log_id		INT	= 999)
AS
BEGIN
...
DECLARE @category			VARCHAR(20)			= 'Eligibility'
       ,@sql				VARCHAR(8000)
...
--*************************************************************************************************
IF OBJECT_ID('tempdb.dbo.#eligibility_ids') IS NOT NULL
	BEGIN DROP TABLE #eligibility_ids END

CREATE TABLE #eligibility_ids
      (ELIGIBILITY_ID					NUMERIC(18,0))
...
--*************************************************************************************************
INSERT INTO #table_deletes (table_name, check_ids) VALUES ('ELIGIBILITY', 0)
INSERT INTO #table_deletes (table_name, check_ids) VALUES ('ELIGIBILITY_BENEFIT', 1)
INSERT INTO #table_deletes (table_name, check_ids) VALUES ('ELIGIBILITY_COMMENT', 1)
INSERT INTO #table_deletes (table_name, check_ids) VALUES ('ELIGIBILITY_PROVIDER', 1)
INSERT INTO #table_deletes (table_name, check_ids) VALUES ('ELIGIBILITY_RESTRICTION', 1)
INSERT INTO #table_deletes (table_name, check_ids) VALUES ('PANEL_ROSTER', 1)

--*************************************************************************************************
-- Populate the mail IDs to specifically look for and delete
--*************************************************************************************************
INSERT INTO #eligibility_ids
      (ELIGIBILITY_ID)
SELECT X.ELIGIBILITY_ID
  FROM ELIGIBILITY			X
 WHERE X.DELETED			= 'Y'
			...
			IF @check_ids <> ''
				BEGIN
					...
					SELECT @sql = 'DELETE TN '
					            + '  FROM #eligibility_ids X '
							    + '  JOIN ' + @table_name + ' TN '
								+ '    ON X.ELIGIBILITY_ID = TN.ELIGIBILITY_ID'
					BEGIN TRY
						--PRINT @sql
						EXEC (@sql)
						SELECT @records = @@ROWCOUNT
						IF @records <> 0 BEGIN EXEC spPortal_DeleteRecordLogDetail @log_id, @category, 'ELIGIBILITY_ID Match', @table_name, @records END
					...
					BEGIN CATCH
						...
						SELECT @err_msg = ERROR_MESSAGE()
						      ,@err_num = 100
						EXEC spPortal_DeleteRecordLogDetail @log_id, @category, 'ELIGIBILITY_ID Match', @table_name, @records, @err_num, @err_msg

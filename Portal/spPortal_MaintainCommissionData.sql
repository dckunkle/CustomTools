/**************************************************************************************************
Name:       spPortal_MaintainCommissionData
...
EXEC spPortal_MaintainCommissionData 1287
***************************************************************************************************/
CREATE OR ALTER PROCEDURE dbo.spPortal_MaintainCommissionData
      (@days		INT	= 30
	  ,@log_id		INT	= 999)
AS
BEGIN
...
DECLARE @category			VARCHAR(20)			= 'Commission'
       ,@sql				VARCHAR(8000)
...
--*************************************************************************************************
IF OBJECT_ID('tempdb.dbo.#commission_ids') IS NOT NULL
	BEGIN DROP TABLE #commission_ids END

CREATE TABLE #commission_ids
      (COMMISSION_ID				NUMERIC(18,0))
...
--*************************************************************************************************
INSERT INTO #table_deletes (table_name, check_ids) VALUES ('COMMISSION', 0)
INSERT INTO #table_deletes (table_name, check_ids) VALUES ('COMMISSION_DETAIL', 1)

--*************************************************************************************************
-- Populate the mail IDs to specifically look for and delete
--*************************************************************************************************
INSERT INTO #commission_ids
      (COMMISSION_ID)
SELECT X.COMMISSION_ID
  FROM dbo.COMMISSION			X
 WHERE X.DELETED				= 'Y'
			...
			IF @check_ids <> ''
				BEGIN
					...
					SELECT @sql = 'DELETE TN '
					            + '  FROM #commission_ids X '
							    + '  JOIN ' + @table_name + ' TN '
								+ '    ON X.COMMISSION_ID = TN.COMMISSION_ID'
					BEGIN TRY
						--PRINT @sql
						EXEC (@sql)
						SELECT @records = @@ROWCOUNT
						IF @records <> 0 BEGIN EXEC spPortal_DeleteRecordLogDetail @log_id, @category, 'COMMISSION_ID Match', @table_name, @records END
					...
					BEGIN CATCH
						...
						SELECT @err_msg = ERROR_MESSAGE()
						      ,@err_num = 100
						EXEC spPortal_DeleteRecordLogDetail @log_id, @category, 'COMMISSION_ID Match', @table_name, @records, @err_num, @err_msg

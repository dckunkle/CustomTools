/**************************************************************************************************
Name:       spPortal_MaintainClaimData
...
EXEC spPortal_MaintainClaimData 1050
***************************************************************************************************/
CREATE OR ALTER PROCEDURE dbo.spPortal_MaintainClaimData
      (@days		INT	= 30
	  ,@log_id		INT	= 999)
AS
BEGIN
...
DECLARE @category			VARCHAR(20)			= 'Claim'
       ,@sql				VARCHAR(8000)
...
--*************************************************************************************************
INSERT INTO #table_deletes (table_name, check_ids) VALUES ('CLAIM',0)
INSERT INTO #table_deletes (table_name, check_ids) VALUES ('CLAIM_NOTE',1)
INSERT INTO #table_deletes (table_name, check_ids) VALUES ('CLAIM_HCARE_SERVICES',1)
INSERT INTO #table_deletes (table_name, check_ids) VALUES ('CLAIM_PAYMENT',1)
INSERT INTO #table_deletes (table_name, check_ids) VALUES ('CLAIM_PROV_ACCESS',1)
INSERT INTO #table_deletes (table_name, check_ids) VALUES ('CLAIM_REQUEST',1)
INSERT INTO #table_deletes (table_name, check_ids) VALUES ('CLAIM_SUPPLEMENTAL_INFO',1)
INSERT INTO #table_deletes (table_name, check_ids) VALUES ('CLAIM_VALUE',1)
INSERT INTO #table_deletes (table_name, check_ids) VALUES ('SERVICE_LINE',1)
INSERT INTO #table_deletes (table_name, check_ids) VALUES ('CLAIM_PATIENT',1)
			...
			IF @check_ids <> ''
				BEGIN
					...
					SELECT @sql = 'DELETE TN '
					            + '  FROM #claim_ids M '
							    + '  JOIN ' + @table_name + ' TN '
								+ '    ON M.CLAIM_ID = TN.CLAIM_ID'
					BEGIN TRY

						EXEC (@sql)
						SELECT @records = @@ROWCOUNT
						IF @records <> 0 BEGIN EXEC spPortal_DeleteRecordLogDetail @log_id, @category, 'CLAIM_ID Match', @table_name, @records END
					...
					BEGIN CATCH
						...
						SELECT @err_msg = ERROR_MESSAGE()
						      ,@err_num = 100
						EXEC spPortal_DeleteRecordLogDetail @log_id, @category, 'CLAIM_ID Match', @table_name, 0, @err_num, @err_msg

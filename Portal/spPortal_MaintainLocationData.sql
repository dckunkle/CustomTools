/**************************************************************************************************
Name:       spPortal_MaintainLocationData
...
EXEC spPortal_MaintainLocationData 1287
***************************************************************************************************/
CREATE OR ALTER PROCEDURE dbo.spPortal_MaintainLocationData
      (@days		INT	= 30
	  ,@log_id		INT	= 999)
AS
BEGIN
...
DECLARE @category			VARCHAR(20)			= 'Location'
       ,@sql				VARCHAR(8000)
...
--*************************************************************************************************
IF OBJECT_ID('tempdb.dbo.#location_ids') IS NOT NULL
	BEGIN DROP TABLE #location_ids END
...
--*************************************************************************************************
INSERT INTO #table_deletes (table_name, check_ids) VALUES ('LOCATION', 0)
INSERT INTO #table_deletes (table_name, check_ids) VALUES ('LOCATION_HOURS', 1)
INSERT INTO #table_deletes (table_name, check_ids) VALUES ('LOCATION_SERVICES', 1)
			...
			IF @check_ids <> ''
				BEGIN
					...
					SELECT @sql = 'DELETE TN '
					            + '  FROM #location_ids X '
							    + '  JOIN ' + @table_name + ' TN '
								+ '    ON X.LOCATION_ID = TN.LOCATION_ID'
					BEGIN TRY
						--PRINT @sql
						EXEC (@sql)
						SELECT @records = @@ROWCOUNT
						IF @records <> 0 BEGIN EXEC spPortal_DeleteRecordLogDetail @log_id, @category, 'LOCATION_ID Match', @table_name, @records END
					...
					BEGIN CATCH
						...
						SELECT @err_msg = ERROR_MESSAGE()
						      ,@err_num = 100
						EXEC spPortal_DeleteRecordLogDetail @log_id, @category, 'LOCATION_ID Match', @table_name, @records, @err_num, @err_msg

/**************************************************************************************************
Purpose:	Given the screen and the stored procedure, build the code to use CoreAutomation data
            to import into any system

Variables:	screen_gid	- the gid of the Core screen (used to gather defaults, etc)
            method		- the name of the mehtod in the fw.Catalog table (used to look up table_name)
			sp_name		- the name of the stored procedure in Core responsible for creating the data
			entity		- this is the name of the entity, which is typically the first parameter
			              in the stored procedure call

Date--------User----Change-------------------------------------------------------------------------
10/22/2019  DK		Original Version
04/20/2020	DK		Add error trapping, temp table cleanup, WAITFOR, etc.
07/12/2022	DK		Modified for CREATE OR ALTER
***************************************************************************************************/
SET NOCOUNT ON

DECLARE @screen_gid			INT				= 11021
	   ,@method				VARCHAR(128)	= 'BusinessUnitClaimsRollingBalanceCreditDebit'
       ,@sp_name			VARCHAR(128)	= 'prManualCreditDebitAdd'
	   ,@entity				VARCHAR(200)	= 'ManualCreditDebitAdd'
...
Screen:     ' + CONVERT(VARCHAR(200), @screen_gid) + '
Method:     ' + @method + '
Procedure:  ' + @sp_name + '
Entity:     ' + @entity + '
...
***************************************************************************************************/
CREATE OR ALTER PROCEDURE dbo.' + @dc_sp_name + '
...
SET @sql = @sql + 'IF OBJECT_ID(''tempdb.dbo.#Tokens'') IS NOT NULL
	DROP TABLE #Tokens

CREATE TABLE #Tokens
      (token			VARCHAR(200)
	  ,token_order		INT				IDENTITY(1,1))' + @crlf	+@crlf
...
SET @sql = @sql + '		WAITFOR DELAY ''00:00:00.100'';'+ @crlf + @crlf
...
DEALLOCATE ' + @method + '_Cursor' + @crlf + @crlf

IF OBJECT_ID('dbo.spDCAuto_EmailResultsVerbose') IS NULL
    EXEC ('CREATE PROCEDURE dbo.spDCAuto_EmailResultsVerbose AS SELECT 1')
GO
/**************************************************************************************************
Name:       spDCAuto_EmailResultsVerbose
...
***************************************************************************************************
EXEC spDCAuto_EmailResultsVerbose 166, 'dkunkle@evolenthealth.com'
***************************************************************************************************/
ALTER PROCEDURE dbo.spDCAuto_EmailResultsVerbose
     (@log_id				INT
...
DECLARE Results_Cursor CURSOR FOR
 SELECT testcase
	   ...
	   ,SUM(success)	AS success
   FROM #results	R
  GROUP BY testcase, method
  ORDER BY MAX(sort_order)
  
  OPEN Results_Cursor
 FETCH NEXT FROM Results_Cursor
  INTO @test_Case_name,@method, @error, @skip, @success
	...
	BEGIN
		...
		DECLARE Errors_Cursor CURSOR FOR
		 ...
		 FETCH NEXT FROM Errors_Cursor
		  INTO @test_case_name, @method, @record_id, @key1, @key2, @key3, @err_num, @err_msg

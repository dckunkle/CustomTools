IF OBJECT_ID('dbo.spDCAuto_CreateTradingPartnersInbound837Ins') IS NULL
    EXEC ('CREATE PROCEDURE dbo.spDCAuto_CreateTradingPartnersInbound837Ins AS SELECT 1')
GO
/**************************************************************************************************
Name:       spDCAuto_CreateTradingPartnersInbound837Ins
Purpose:    Create tradingpartnersinbound837ins data from CorderAutomation
Method:     TradingPartnersInbound837Ins
Screen GID: 9502
...
***************************************************************************************************
EXEC spDCAuto_CreateTradingPartnersInbound837Ins '100-Config%', 22, 'TradingPartnersInbound837Ins'
***************************************************************************************************/
ALTER PROCEDURE dbo.spDCAuto_CreateTradingPartnersInbound837Ins
...
--*************************************************************************************************
IF OBJECT_ID('tempdb.dbo.#TradingPartnersInbound837Ins') IS NOT NULL
	DROP TABLE #TradingPartnersInbound837Ins

CREATE TABLE #TradingPartnersInbound837Ins
      (SearchID                     VARCHAR(200)
      ,i_entity_name                VARCHAR(50)       DEFAULT('837I_Inbound')
      ,i_Trading_partner_gid        VARCHAR(50)       DEFAULT('0')
      ,i_Key_Transaction_type       VARCHAR(80)       DEFAULT('837I')
      ,i_Key_Direction              VARCHAR(50)       DEFAULT('I')
      ,i_Trxn_SID                   VARCHAR(50)       DEFAULT('0')
      ,i_Key_5                      VARCHAR(50)       DEFAULT('837I_Inbound')
      ...
      ,i_Transaction_Type           VARCHAR(50)       DEFAULT('837I')
...
--*************************************************************************************************
INSERT INTO #TradingPartnersInbound837Ins
      (SearchID
...
SELECT SearchID
      ...
      ,ISNULL(dbo.fnDCAuto_GetDropdownValue([ValidateAttendingProviderMedicaidID]), 'N')
	  ,ISNULL([SequestrationAdjustmentID], '')
      ,ISNULL([RecordID], '')
      ,ISNULL([gid], '')
  FROM COREAUTO.CoreAutomation.dbo.TD_TradingPartnersInbound837Ins
...
-- Update the user
--*************************************************************************************************
UPDATE #TradingPartnersInbound837Ins
   SET i_user_id  = @user

--*************************************************************************************************
-- Create the data
--*************************************************************************************************
DECLARE TradingPartnersInbound837Ins_Cursor CURSOR FOR
 SELECT SearchID
       ,i_entity_name
       ...
       ,record_id
       ,static_gid
   FROM #TradingPartnersInbound837Ins

   OPEN TradingPartnersInbound837Ins_Cursor
  FETCH NEXT FROM TradingPartnersInbound837Ins_Cursor
   INTO @SearchID
       ,@i_entity_name
       ,@i_Trading_partner_gid
       ,@i_Key_Transaction_type
...
CLOSE TradingPartnersInbound837Ins_Cursor
DEALLOCATE TradingPartnersInbound837Ins_Cursor

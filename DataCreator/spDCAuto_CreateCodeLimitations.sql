IF OBJECT_ID('dbo.spDCAuto_CreateCodeLimitations') IS NULL
    EXEC ('CREATE PROCEDURE dbo.spDCAuto_CreateCodeLimitations AS SELECT 1')
GO
/**************************************************************************************************
Name:       spDCAuto_CreateCodeLimitations
Purpose:    Create codelimitations data from CorderAutomation
Method:     CodeLimitations
Screen GID: 195
...
***************************************************************************************************
EXEC spDCAuto_CreateCodeLimitations '100-Config%', 22, 'CodeLimitations'
***************************************************************************************************/
ALTER PROCEDURE dbo.spDCAuto_CreateCodeLimitations
...
--*************************************************************************************************
IF OBJECT_ID('tempdb.dbo.#CodeLimitations') IS NOT NULL
	DROP TABLE #CodeLimitations

CREATE TABLE #CodeLimitations
      (SearchID            VARCHAR(200)
      ,iEntityName         VARCHAR(50)       DEFAULT('Coverage_Strategy')
...
SELECT SearchID
      ...
      ,ISNULL([*CodeLimitationsDesc], '')
      ,ISNULL([CopyFrmCodeLimitationsID], '')
      ,ISNULL([RecordID], '')
      ,ISNULL([gid], '')
  FROM COREAUTO.CoreAutomation.dbo.TD_CodeLimitations
...
-- Update the user
--*************************************************************************************************
UPDATE #CodeLimitations
   SET iUserID  = @user

--*************************************************************************************************
-- Create the data
--*************************************************************************************************
DECLARE CodeLimitations_Cursor CURSOR FOR
 SELECT SearchID
       ,iEntityName
       ,iKey_1_field
       ,iKey_2_field
       ...
       ,record_id
       ,static_gid
   FROM #CodeLimitations

   OPEN CodeLimitations_Cursor
  FETCH NEXT FROM CodeLimitations_Cursor
   INTO @SearchID
       ,@iEntityName
       ,@iKey_1_field
       ,@iKey_2_field
			...
			BEGIN
				...
				UPDATE dbo.Entity_Names 
				 ...
				 WHERE record_status			= 'A'
				   AND entity_identifier		= 'Coverage_Strategy'
...
CLOSE CodeLimitations_Cursor
DEALLOCATE CodeLimitations_Cursor

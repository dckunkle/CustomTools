IF OBJECT_ID('dbo.spDCAuto_CreateGroupAssignment') IS NULL
    EXEC ('CREATE PROCEDURE dbo.spDCAuto_CreateGroupAssignment AS SELECT 1')
GO
/**************************************************************************************************
Name:       spDCAuto_CreateGroupAssignment
Purpose:    Create groupassignment data from CorderAutomation
Method:     GroupAssignment
Screen GID: 24
...
***************************************************************************************************
EXEC spDCAuto_CreateGroupAssignment '100-Config%', 22, 'GroupAssignment'
***************************************************************************************************/
ALTER PROCEDURE dbo.spDCAuto_CreateGroupAssignment
...
DECLARE @i_entity_name        VARCHAR(100)
       ...
       ,@i_group_gid          VARCHAR(100)
       ,@i_member_gid         VARCHAR(50)
       ,@i_agency_gid         VARCHAR(100)
       ,@i_cr_gid             VARCHAR(50)
       ,@i_grouper_gid        VARCHAR(100)
       ,@i_key_7_field        VARCHAR(50)
       ,@i_key_8_field        VARCHAR(50)
       ,@i_key_9_field        VARCHAR(50)
       ,@i_assign_type        VARCHAR(100)
...
--*************************************************************************************************
IF OBJECT_ID('tempdb.dbo.#GroupAssignment') IS NOT NULL
	DROP TABLE #GroupAssignment

CREATE TABLE #GroupAssignment
      (SearchID             VARCHAR(200)
      ,i_entity_name        VARCHAR(100)      DEFAULT('BR_GR')
      ,i_assign_gid         VARCHAR(50)       DEFAULT('0')
      ,i_group_gid          VARCHAR(100)      DEFAULT('0')
      ,i_member_gid         VARCHAR(50)       DEFAULT('0')
      ,i_agency_gid         VARCHAR(100)      DEFAULT('0')
      ,i_cr_gid             VARCHAR(50)       DEFAULT('0')
      ,i_grouper_gid        VARCHAR(100)      DEFAULT('0')
      ,i_key_7_field        VARCHAR(50)       DEFAULT('0')
      ,i_key_8_field        VARCHAR(50)       DEFAULT('0')
      ,i_key_9_field        VARCHAR(50)       DEFAULT('0')
      ,i_assign_type        VARCHAR(100)      DEFAULT('GROUP')
...
SELECT SearchID
      ...
      ,ISNULL([RecordID], '')
      ,ISNULL([gid], '')
  FROM COREAUTO.CoreAutomation.dbo.TD_GroupAssignments
...
-- Update the user
--*************************************************************************************************
UPDATE #GroupAssignment
   SET iUserID  = @user

--*************************************************************************************************
-- Create the data
--*************************************************************************************************
DECLARE GroupAssignment_Cursor CURSOR FOR
 SELECT SearchID
       ,i_entity_name
       ,i_assign_gid
       ,i_group_gid
   ...
   OPEN GroupAssignment_Cursor
  FETCH NEXT FROM GroupAssignment_Cursor
   INTO @SearchID
       ,@i_entity_name
       ,@i_assign_gid
       ,@i_group_gid
...
CLOSE GroupAssignment_Cursor
DEALLOCATE GroupAssignment_Cursor

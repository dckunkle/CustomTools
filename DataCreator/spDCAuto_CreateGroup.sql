IF OBJECT_ID('dbo.spDCAuto_CreateGroup') IS NULL
    EXEC ('CREATE PROCEDURE dbo.spDCAuto_CreateGroup AS SELECT 1')
GO
/**************************************************************************************************
Name:       spDCAuto_CreateGroup
Purpose:    Create group data from CorderAutomation
Method:     Group
Screen GID: 14
...
01/06/2019	DK				Changed temp table name from #Group to #GroupToAdd to avoid naming 
                            conflicts in downstream stored procedures
---------------------------------------------------------------------------------------------

***************************************************************************************************
EXEC spDCAuto_CreateGroup '100-Config%', 22, 'Group'
***************************************************************************************************/
ALTER PROCEDURE dbo.spDCAuto_CreateGroup
...
DECLARE @pattern					VARCHAR(200)
	   ...
	   ,@xml						XML				= '' --'<root><results status_code="800" screen_gid="575" Value="N" ErrorType=""/></root>'
...
--*************************************************************************************************
IF OBJECT_ID('tempdb.dbo.#GroupToAdd') IS NOT NULL
	DROP TABLE #GroupToAdd

CREATE TABLE #GroupToAdd
      (SearchID                    VARCHAR(200)
      ,i_entity_name               VARCHAR(50)       DEFAULT('Groups')
...
SELECT SearchID
      ...
      ,ISNULL([RecordID], '')
      ,ISNULL([gid], '')
  FROM COREAUTO.CoreAutomation.dbo.TD_Group
			...
			BEGIN
				...
				UPDATE dbo.GroupTree
				   SET Child_gid				= @static_gid
				      ,Parent_gid				= @static_gid
					  ,Super_gid				= @static_gid
				 WHERE Child_gid				= @current_gid
				   AND Parent_gid				= @current_gid
				   AND Super_gid				= @current_gid

IF OBJECT_ID('dbo.spDCAuto_CreateCopayLevels') IS NULL
    EXEC ('CREATE PROCEDURE dbo.spDCAuto_CreateCopayLevels AS SELECT 1')
GO
/**************************************************************************************************
Name:       spDCAuto_CreateCopayLevels
Purpose:    Create copaylevels data from CorderAutomation
Method:     CopayLevels
Screen GID: 196
...
***************************************************************************************************
EXEC spDCAuto_CreateCopayLevels '100-Config%', 22, 'CopayLevels'
***************************************************************************************************/
ALTER PROCEDURE dbo.spDCAuto_CreateCopayLevels
...
--*************************************************************************************************
IF OBJECT_ID('tempdb.dbo.#CopayLevels') IS NOT NULL
	DROP TABLE #CopayLevels

CREATE TABLE #CopayLevels
      (iEntityName         VARCHAR(50)       DEFAULT('Copay_Strategy')
...
--*************************************************************************************************
INSERT INTO #CopayLevels
      (iNewStrategyID
      ,iNewStrategyDesc
      ,iCopyStrategyID
      ,record_id
      ,static_gid)
SELECT ISNULL([*NewCopayStratID], '')
      ,ISNULL([*NewCopayStratDesc], '')
      ,ISNULL([CopyFromCopayStratID], '')
      ,ISNULL([RecordID], '')
      ,ISNULL([gid], '')
  FROM COREAUTO.CoreAutomation.dbo.TD_CopayLevels
...
-- Create the data
--*************************************************************************************************
DECLARE CopayLevels_Cursor CURSOR FOR
 SELECT iEntityName
       ,iKey_1_field
       ,iKey_2_field
       ,iKey_3_field
   ...
   OPEN CopayLevels_Cursor
  FETCH NEXT FROM CopayLevels_Cursor
   INTO @iEntityName
       ,@iKey_1_field
       ,@iKey_2_field
       ,@iKey_3_field
			...
			BEGIN
				...
				UPDATE dbo.Entity_Names 
				 ...
				 WHERE record_status			= 'A'
				   AND entity_identifier		= 'COPAY_STRATEGY'
...
CLOSE CopayLevels_Cursor
DEALLOCATE CopayLevels_Cursor

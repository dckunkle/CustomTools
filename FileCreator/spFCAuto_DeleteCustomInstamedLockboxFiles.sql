IF OBJECT_ID('dbo.spFCAuto_DeleteCustomInstamedLockboxFiles') IS NULL
    EXEC ('CREATE PROCEDURE dbo.spFCAuto_DeleteCustomInstamedLockboxFiles AS SELECT 1')
GO
/**************************************************************************************************
Name:       spFCAuto_DeleteCustomInstamedLockboxFiles
Purpose:    Delete data that was loaded by a lockbox file

Date        User            Change
---------------------------------------------------------------------------------------------
03/11/2021	DK				Original procedure
---------------------------------------------------------------------------------------------

***************************************************************************************************
EXEC spFCAuto_DeleteCustomInstamedLockboxFiles 'Dev-Lockbox'
***************************************************************************************************/
ALTER PROCEDURE dbo.spFCAuto_DeleteCustomInstamedLockboxFiles
...
SELECT @filename							= C.filename
 ...
 WHERE C.method_name						= 'InstamedLockbox'
		...
		BEGIN TRY
			...
			WHILE @@FETCH_STATUS = 0
				BEGIN
					...
					EXEC spFCAuto_DeleteCustomLockboxFile @file_name, @err_num OUTPUT, @err_msg OUTPUT, @type_id
					FETCH NEXT FROM Delete_Files
					 INTO @file_name
